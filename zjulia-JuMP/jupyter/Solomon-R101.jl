...
n::Int = size(data0, 1) - 1

data::Matrix{Int} = zeros(Int, n+2, 7)
data[1:n+1, :] = data0
data[n+2, :] = data0[1,:]
data[n+2, 1] = n+2

X::Vector{Int} = data[:,2] # XCOORD
Y::Vector{Int} = data[:,3] # YCOORD
d::Vector{Int} = data[:,4] # DEMAND
a::Vector{Int} = data[:,5] # READY TIME
b::Vector{Int} = data[:,6] # DUE DATE
st::Vector{Int} = data[:,7] # SERVICE TIME
q::Int = 200

data0 = zeros(Float64, 0,0)
data  = zeros(Int, 0,0)

println("Solomon-R101")
